<template>
    <div class="_fc-table-form" :class="{'_fc-disabled': disabled}">
        ...
        <van-button type="primary" size="mini" class="fc-clock" v-if="!disabled" @click="batchImport">
            <i class="fc-icon icon-import" style="font-weight: 700;"></i>
            {{ formCreateInject.t('batchImport') || '批量导入' }}
        </van-button>
        <van-button type="primary" size="mini" class="fc-clock" v-if="modelValue && modelValue.length" @click="batchExport">
